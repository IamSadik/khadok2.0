...
document.getElementById('place-pickup-btn').addEventListener('click', async () => {
    ...
    try {
        const response = await fetch('/api/order/delivery', {
            method: 'POST',
            headers: {
        ...
        if (response.ok && responseData.success) {
            ...
            window.location.href = 'map.html';  // Redirect to confirmation page
        } else {
            throw new Error(responseData.message || 'Failed to place delivery order.');
        }
    } catch (error) {
